...
class Track1ForOrganization:
    ...
    def get_response_content(self):
        ...
        if self.http_method == 'PUT':
            organization_id = self.payload['id']
            req_path = f'/{self.resource}/{organization_id}'
    ...
    def generate_organization_resource(self):
        if self.http_method == 'PUT':
            organization_class = Organization(self.payload['id'])
        else:
        ...
        organization_class.set_identifiers(self.payload['identifier'])

        organization_class.set_type_coding(self.payload['type_coding'])

        organization_class.set_name(self.payload['name'])

        organization_class.set_telecom(self.payload['telecom'])

        organization_class.create()

        print(organization_class.payload_template)
