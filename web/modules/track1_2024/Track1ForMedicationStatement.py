...
class Track1ForMedicationStatement:
    ...
    def get_response_content(self):
        ...
        json_payload = self.generate_medication_statement_resource()
    ...
    def generate_medication_statement_resource(self):
        if self.http_method == 'PUT':
            medication_statement_class = MedicationStatement(self.payload['id'])
        else:
            medication_statement_class = MedicationStatement()

        profile_urls = self.payload['profile_urls']

        medication_statement_class.set_profile_urls(profile_urls)

        medication_statement_class.set_status(self.payload['status'])

        medication_statement_class.set_category_coding(self.payload['category_coding'])
        medication_statement_class.set_category_text(self.payload['category_text'])

        medication_statement_class.set_medication_codeable_concept_coding(self.payload['medication_codeable_concept_coding'])
        medication_statement_class.set_medication_codeable_concept_text(self.payload['medication_codeable_concept_text'])

        medication_statement_class.set_subject(self.payload['subject'])

        medication_statement_class.set_effective_date_time(self.payload['effective_date_time'])

        medication_statement_class.set_date_asserted(self.payload['date_asserted'])

        medication_statement_class.set_reason_code_coding(self.payload['reason_code_coding'])
        medication_statement_class.set_reason_code_text(self.payload['reason_code_text'])

        medication_statement_class.set_note(self.payload['note'])

        medication_statement_class.set_dosage_text(self.payload['dosage_text'])
        medication_statement_class.set_dosage_timing_repeat(self.payload['dosage_timing_repeat'])
        medication_statement_class.set_dosage_route_coding(self.payload['dosage_route_coding'])
        medication_statement_class.set_dosage_route_text(self.payload['dosage_route_text'])

        medication_statement_class.create()

        return medication_statement_class.payload_template
