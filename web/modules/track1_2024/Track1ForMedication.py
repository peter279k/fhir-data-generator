...
class Track1ForMedication:
    ...
    def get_response_content(self):
        ...
        json_payload = self.generate_medication_resource()
    ...
    def generate_medication_resource(self):
        if self.http_method == 'PUT':
            medication_class = Medication(self.payload['id'])
        else:
        ...
        medication_class.set_code_coding(self.payload['code_coding'])
        medication_class.set_code_text(self.payload['code_text'])

        medication_class.set_form_coding(self.payload['form_coding'])
        medication_class.set_form_text(self.payload['form_text'])

        medication_class.create()

        return medication_class.payload_template
