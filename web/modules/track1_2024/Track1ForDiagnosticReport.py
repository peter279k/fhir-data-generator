...
class Track1ForDiagnosticReport:
    ...
    def get_response_content(self):
        ...
        json_payload = self.generate_diagnostic_report_resource()
    ...
    def generate_diagnostic_report_resource(self):
        if self.http_method == 'PUT':
            diagnostic_report_class = DiagnosticReport(self.payload['id'])
        else:
            diagnostic_report_class = DiagnosticReport()

        profile_urls = self.payload['profile_urls']
        diagnostic_report_class.set_profile_urls(profile_urls)

        diagnostic_report_class.set_status(self.payload['status'])

        diagnostic_report_class.set_category_coding(self.payload['category_coding'])
        diagnostic_report_class.set_category_text(self.payload['category_text'])

        diagnostic_report_class.set_code_coding(self.payload['code_coding'])
        diagnostic_report_class.set_code_text(self.payload['code_text'])

        diagnostic_report_class.set_subject(self.payload['subject'])

        diagnostic_report_class.set_effective_datetime(self.payload['effective_datetime'])

        diagnostic_report_class.set_issued(self.payload['issued'])

        diagnostic_report_class.set_performer(self.payload['performer'])

        diagnostic_report_class.set_result(self.payload['result'])
